...
class AnimatablePathValue implements IAnimatablePathValue {
  ...
  AnimatablePathValue(Object json, LottieComposition composition)
    ...
    if (hasKeyframes(json)) {
      ...
      JSONArray initialPointJson;
      if (json instanceof JSONArray) {
        initialPointJson = (JSONArray) json;
      } else {
        initialPointJson = ((JSONObject) json).getJSONArray("k");
      }
      initialPoint = JsonUtils.pointFromJsonArray(initialPointJson, composition.getScale());
    }
  }
